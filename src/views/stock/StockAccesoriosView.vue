...
const filteredItems = computed(() => {
  ...
  const ubicacion = filtroUbicacion.value.trim().toLowerCase()
  if (ubicacion) list = list.filter((i) => (i.ubicacion || '').toLowerCase().includes(ubicacion))
  return list
})

const hasActiveFilters = computed(() =>
  filtroEstado.value || filtroNombre.value || filtroMarca.value ||
  filtroModelo.value || filtroNumeroSerie.value || filtroUbicacion.value
)
...
async function load() {
  ...
  error.value = ''
  try {
    items.value = await getAccesorios()
  } catch (e) {
...
<template>
  <div class="qnt-page">
    ...
    <PageHeader title="Accesorios" :subtitle="`${items.length} accesorios registrados`" />
    ...
    <div class="qnt-toolbar">
      ...
      <input v-model="filtroUbicacion" type="text" class="qnt-input filter-sm" placeholder="Ubicación" />
    ...
    <div v-if="loading" class="qnt-state qnt-state--row">
      <span class="qnt-spinner" /> Cargando accesorios…
    </div>
    ...
    <div v-else-if="filteredItems.length === 0" class="qnt-state">
      <Package style="width:40px;height:40px;opacity:.25" />
      <p>{{ hasActiveFilters ? 'Sin resultados con los filtros aplicados.' : 'No hay accesorios registrados.' }}</p>
      <button v-if="hasActiveFilters" class="qnt-btn qnt-btn--secondary qnt-btn--sm" @click="clearFilters">
        Limpiar filtros
      </button>
    </div>
    ...
    <div v-else class="equip-grid">
      ...
      >
        <div class="equip-card__img-wrap">
          <img src="/Images/Accesorios.jpg" alt="Accesorio" class="equip-card__img" />
          <StatusBadge :estado="item.estado" class="equip-card__badge" />
        </div>
        <div class="equip-card__body">
          <p class="equip-card__serial">
            <template v-if="item.numeroSerie">{{ item.numeroSerie }}</template>
            <template v-else-if="item.cantidad != null && item.unidadMedida">{{ item.cantidad }} {{ item.unidadMedida }}</template>
            <template v-else>{{ `#${item.id}` }}</template>
          </p>
          <p class="equip-card__title">{{ item.nombre || [item.marca, item.modelo].filter(Boolean).join(' ') || '—' }}</p>
          <p v-if="item.ubicacion" class="equip-card__location">{{ item.ubicacion }}</p>
        </div>
      </button>
    </div>
  </div>
