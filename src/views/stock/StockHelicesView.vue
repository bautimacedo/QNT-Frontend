...
import { Wind, Search, RefreshCw, Plus } from 'lucide-vue-next'
import StatusBadge from '../../components/ui/StatusBadge.vue'
import PageHeader  from '../../components/ui/PageHeader.vue'

const router = useRouter()
const TIPO        = 'helices'
const LISTA_ROUTE = '/home/stock/helices'
...
<template>
  <div class="qnt-page">
    ...
    <PageHeader title="Hélices" :subtitle="`${items.length} equipos registrados`">
      <template #actions>
    ...
    <div v-if="loading" class="qnt-state qnt-state--row">
      <span class="qnt-spinner" /> Cargando hélices…
    </div>
    ...
    <div v-else-if="filteredItems.length === 0" class="qnt-state">
      <Wind style="width:40px;height:40px;opacity:.25" />
      <p>{{ hasActiveFilters ? 'Sin resultados con los filtros aplicados.' : 'No hay hélices registradas.' }}</p>
      <button v-if="hasActiveFilters" class="qnt-btn qnt-btn--secondary qnt-btn--sm" @click="clearFilters">
        Limpiar filtros
      </button>
    </div>
    ...
    <div v-else class="equip-grid">
      ...
      >
        <div class="equip-card__img-wrap">
          <img src="/Images/helices.jpg" alt="Hélice" class="equip-card__img" />
  ...
  <Teleport to="body">
    <div v-if="createModal.open" class="modal-overlay" @click.self="closeCreate">
      <div class="modal-card">
        <h2 class="modal-title">Agregar hélice manualmente</h2>
        <div class="form-group">
          <label>Nombre <span style="color:#dc2626">*</span></label>
          <input v-model="createModal.nombre" type="text" class="form-input" placeholder="Ej: Hélice Par 1 EFO-Q1" />
        </div>
        <div class="form-group">
          <label>Marca</label>
        ...
        <div class="modal-actions">
          ...
          <button type="button" class="btn-primary" :disabled="createModal.loading" @click="saveCreate">
            {{ createModal.loading ? 'Guardando…' : 'Agregar hélice' }}
          </button>
        </div>
      </div>
    </div>
