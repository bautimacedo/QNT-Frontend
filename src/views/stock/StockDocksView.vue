...
import { HardDrive, Search, RefreshCw, Plus } from 'lucide-vue-next'
...
<template>
  <div class="qnt-page">
    ...
    <PageHeader title="Docks" :subtitle="`${items.length} equipos registrados`">
      <template #actions>
    ...
    <div v-if="loading" class="qnt-state qnt-state--row">
      <span class="qnt-spinner" /> Cargando docks…
    </div>
    ...
    <div v-else-if="filteredItems.length === 0" class="qnt-state">
      <HardDrive style="width:40px;height:40px;opacity:.25" />
      <p>{{ hasActiveFilters ? 'Sin resultados con los filtros aplicados.' : 'No hay docks registrados.' }}</p>
      <button v-if="hasActiveFilters" class="qnt-btn qnt-btn--secondary qnt-btn--sm" @click="clearFilters">
        Limpiar filtros
      </button>
    </div>
    ...
    <div v-else class="equip-grid">
      ...
      >
        <div class="equip-card__img-wrap">
          <img src="/Images/dock.png" alt="Dock" class="equip-card__img" />
  ...
  <Teleport to="body">
    <div v-if="createModal.open" class="modal-overlay" @click.self="closeCreate">
      <div class="modal-card">
        <h2 class="modal-title">Agregar dock manualmente</h2>
        <div class="form-group">
          <label>Nombre <span style="color:#dc2626">*</span></label>
          <input v-model="createModal.nombre" type="text" class="form-input" placeholder="Ej: Dock EFO-1" />
        </div>
        <div class="form-group">
          <label>Marca</label>
        ...
        <div class="form-group">
          <label>Modelo</label>
          <input v-model="createModal.modelo" type="text" class="form-input" placeholder="Ej: Dock 3" />
        </div>
        <div class="form-group">
        ...
        <div class="modal-actions">
          ...
          <button type="button" class="btn-primary" :disabled="createModal.loading" @click="saveCreate">
            {{ createModal.loading ? 'Guardando…' : 'Agregar dock' }}
          </button>
        </div>
      </div>
    </div>
