...
const camposDetalle = computed(() => {
  ...
  return [
    ...
    { label: 'Nº licencia', value: o.numLicencia },
    { label: 'Compra', value: o.compra ? (o.compra.id ?? o.compra) : (o.compraId ?? '—') },
    { label: 'Fecha compra', value: o.fechaCompra },
    { label: 'Caducidad', value: o.caducidad },
    { label: 'Versión', value: o.version },
    { label: 'Activo', value: o.activo != null ? (o.activo ? 'Sí' : 'No') : '—' },
  ]
})
...
async function load() {
  loading.value = true
  error.value = ''
  try { item.value = await getLicencia(route.params.id) } catch (e) {
