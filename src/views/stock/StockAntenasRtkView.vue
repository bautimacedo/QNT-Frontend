...
import { Radio, Search, RefreshCw } from 'lucide-vue-next'
import StatusBadge from '../../components/ui/StatusBadge.vue'
import PageHeader  from '../../components/ui/PageHeader.vue'

const router = useRouter()
const TIPO        = 'antenas-rtk'
const LISTA_ROUTE = '/home/stock/antenas-rtk'
...
<template>
  <div class="qnt-page">
    <nav class="qnt-breadcrumb">
      ...
      <span>Antenas RTK</span>
    </nav>

    <PageHeader title="Antenas RTK" :subtitle="`${items.length} equipos registrados`" />
    ...
    <div v-if="loading" class="qnt-state qnt-state--row">
      <span class="qnt-spinner" /> Cargando antenas RTK…
    </div>
    ...
    <div v-else-if="filteredItems.length === 0" class="qnt-state">
      <Radio style="width:40px;height:40px;opacity:.25" />
      <p>{{ hasActiveFilters ? 'Sin resultados con los filtros aplicados.' : 'No hay antenas RTK registradas.' }}</p>
      <button v-if="hasActiveFilters" class="qnt-btn qnt-btn--secondary qnt-btn--sm" @click="clearFilters">
        Limpiar filtros
      </button>
    </div>
    ...
    <div v-else class="equip-grid">
      ...
      >
        <div class="equip-card__img-wrap">
          <img src="/Images/rtk.png" alt="Antena RTK" class="equip-card__img" />
