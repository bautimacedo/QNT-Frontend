...
async function load() {
  loading.value = true
  error.value = ''
  try { items.value = await getLicencias() } catch (e) { error.value = e.message || 'Error al cargar los datos.'; items.value = [] }
  ...
  <div class="stock-list-page">
    ...
    <template v-else>
      <div class="filters">
        ...
        <input v-model="filtroNumLicencia" type="text" placeholder="Nº licencia" class="filter-input" />
      ...
      <div v-else class="cards-grid">
        <button v-for="item in filteredItems" :key="item.id" type="button" class="item-card" @click="verDetalle(item.id)">
          ...
          <div class="item-card__body">
            ...
            <div v-if="item.numLicencia" class="item-card__meta">{{ item.numLicencia }}</div>
            <div class="item-card__estado">{{ item.activo ? 'Activo' : 'Inactivo' }}</div>
          </div>
        </button>
      </div>
    </template>
