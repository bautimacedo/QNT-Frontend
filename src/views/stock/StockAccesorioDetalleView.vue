...
const camposDetalle = computed(() => {
  ...
  const list = [
    ...
    { label: 'Fecha compra', value: o.fechaCompra },
    { label: 'Cantidad', value: o.cantidad != null ? String(o.cantidad) : '—' },
    { label: 'Unidad de medida', value: o.unidadMedida },
    { label: 'Descripción', value: o.descripcion },
    { label: 'Ubicación', value: o.ubicacion },
    { label: 'Site', value: o.site ? (o.site.nombre ?? o.site.id ?? '—') : '—' },
  ]
  // Cualquier campo extra que devuelva el backend
  Object.keys(o).forEach((k) => {
    if (!['id', 'nombre', 'marca', 'modelo', 'numeroSerie', 'estado', 'fechaCompra', 'cantidad', 'unidadMedida', 'descripcion', 'ubicacion', 'site'].includes(k)) {
      const v = o[k]
  ...
  <div class="stock-detail-page">
    ...
    <template v-else-if="item">
      <header class="page-header">
        <h1 class="page-title">Detalle de {{ TITULO }}</h1>
      </header>
      <div class="detail-card">
        <div class="detail-card__image-wrap">
          <img :src="IMAGEN" :alt="TITULO" class="detail-card__image" />
        </div>
