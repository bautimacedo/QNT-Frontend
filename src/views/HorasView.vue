...
const totalActividades = computed(() =>
  resumenFiltrado.value.reduce((acc, r) => acc + Number(r.cantidadRegistros || 0), 0)
)

// ── Modales ──────────────────────────────────────────────────────────────
const formModal = ref({ open: false, editing: null, loading: false, fecha: '', descripcion: '', errors: {}, apiError: '', ampliando: false })
...
function openForm() {
  formModal.value = { open: true, editing: null, loading: false, fecha: hoyISO(), descripcion: '', errors: {}, apiError: '', ampliando: false }
}
function openEdit(r) {
  formModal.value = { open: true, editing: r, loading: false, fecha: r.fecha, descripcion: r.descripcion || '', errors: {}, apiError: '', ampliando: false }
}
...
async function loadAll() {
  ...
  try {
    ...
    error.value = e.message || 'No se pudieron cargar las actividades.'
  } finally {
    loading.value = false
  }
}
...
function validarFecha(fecha) {
  if (!fecha) return 'Requerida'
  if (fecha > hoyISO()) return 'No puede ser futura'
  return null
}

async function save() {
  const errs = {}
  const ef = validarFecha(formModal.value.fecha)
  if (ef) errs.fecha = ef
  if (!formModal.value.descripcion.trim()) errs.descripcion = 'Describí la actividad'
  formModal.value.errors = errs
  if (Object.keys(errs).length) return

  formModal.value.loading = true
  formModal.value.apiError = ''
  const body = { fecha: formModal.value.fecha, descripcion: formModal.value.descripcion }
  try {
...
async function correrAsistente(fn, vacioMsg) {
  ...
  try {
    ...
    asistModal.value.borradores = borradores.map(b => ({ fecha: b.fecha, descripcion: b.descripcion }))
  } catch (e) {
...
function generarBorradores() {
  correrAsistente(asistenteHoras, 'El asistente no pudo extraer actividades (¿la IA está configurada? ¿el texto es claro?).')
}
...
async function confirmarBorradores() {
  const validos = asistModal.value.borradores.filter(b => !validarFecha(b.fecha) && b.descripcion.trim())
  ...
  try {
    for (const b of validos) {
      await crearHora({ fecha: b.fecha, descripcion: b.descripcion })
    }
    showToast(`${validos.length} actividad(es) guardada(s).`)
    closeAsist()
    loadAll()
  } catch (e) {
...
function exportarCSV() {
  ...
  if (!filas.length) { showToast('No hay actividades para exportar.'); return }
  const headers = ['Persona', 'Fecha', 'Actividad']
  const esc = v => `"${String(v ?? '').replace(/"/g, '""')}"`
  const lineas = filas.map(r => [nombreCompleto(r.autor), r.fecha, r.descripcion || ''].map(esc).join(','))
  const csv = [headers.join(','), ...lineas].join('\n')
  const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `actividades-${filtroMes.value}.csv`
  a.click()
...
<template>
  <div class="qnt-page">
    <div class="qnt-page-header">
      <div>
        <h1 class="qnt-page-title">Registro de Actividades</h1>
        <p class="qnt-page-subtitle">{{ totalActividades }} actividad{{ totalActividades !== 1 ? 'es' : '' }} en el período</p>
      </div>
      <div class="header-actions">
        <button class="qnt-btn qnt-btn--ghost" @click="openAsist">✨ Asistente</button>
        <button class="qnt-btn qnt-btn--primary" @click="openForm">+ Cargar actividad</button>
      </div>
    </div>
    ...
    <div v-if="resumenFiltrado.length" class="resumen-row">
      <div v-for="r in resumenFiltrado" :key="r.autorId" class="resumen-card">
        ...
        <div class="resumen-card__horas">{{ r.cantidadRegistros }} <span>actividad{{ r.cantidadRegistros !== 1 ? 'es' : '' }}</span></div>
      </div>
    </div>
    ...
    <div v-else-if="registrosFiltrados.length === 0" class="qnt-empty">No hay actividades cargadas en este período.</div>

    <div v-else class="qnt-table-wrap">
      <table class="qnt-table">
        <thead>
          <tr><th>Persona</th><th>Fecha</th><th>Actividad</th><th>Acciones</th></tr>
        </thead>
        <tbody>
    ...
    <Teleport to="body">
      <Transition name="qnt-modal">
        <div v-if="formModal.open" class="qnt-modal-overlay">
          <div class="qnt-modal">
            <h3 class="qnt-modal__title">{{ formModal.editing ? 'Editar actividad' : 'Cargar actividad' }}</h3>
            <div class="qnt-field">
            ...
            <div class="qnt-field">
              ...
              <textarea v-model="formModal.descripcion" class="qnt-textarea" rows="4" placeholder="Qué actividad realizaste…" :disabled="formModal.loading || formModal.ampliando" />
        ...
        <div v-if="asistModal.open" class="qnt-modal-overlay">
          <div class="qnt-modal qnt-modal--xl">
            ...
            <p class="qnt-modal__subtitle">
              <b>Generar borradores</b>: contá lo que hiciste y la IA lo ordena fielmente (ej. "ayer testeos en el dock de CL, hoy módulo de actividades").<br>
              <b>Asistente +</b>: poné un tema/período y la IA <b>propone</b> actividades plausibles del proyecto para que elijas. Siempre revisás antes de guardar.
            </p>
            <textarea v-model="asistModal.texto" class="qnt-textarea" rows="3" placeholder="Escribí tus actividades, o un tema/período para que el Asistente + proponga…" :disabled="asistModal.loading || asistModal.saving" />
            ...
            <div v-if="asistModal.borradores.length" class="borradores">
              <table class="qnt-table">
                <thead><tr><th>Fecha</th><th>Actividad</th><th></th></tr></thead>
                <tbody>
      ...
      <Transition name="qnt-modal">
        <div v-if="confirmModal.open" class="qnt-modal-overlay">
          <div class="qnt-modal qnt-modal--sm">
            <h3 class="qnt-modal__title">¿Eliminar actividad?</h3>
            <p class="qnt-modal__subtitle">Se eliminará la actividad del {{ formatFecha(confirmModal.registro?.fecha) }}.</p>
            <div class="qnt-modal__actions">
...
.td-desc { max-width: 480px; color: #475569; }
