...
test('estado vacío muestra mensaje cuando no hay hélices', async ({ page }) => {
  const card = page.locator('.equip-card').first()
  if (await card.count() === 0) {
    await expect(page.locator('text=No hay hélices registradas')).toBeVisible()
    return
  }
...
test('detalle de hélice muestra estado', async ({ page }) => {
  const card = page.locator('button.equip-card, .equip-card').first()
  if (await card.count() === 0) { test.skip(); return }
  await card.click()
  await page.waitForLoadState('networkidle')
