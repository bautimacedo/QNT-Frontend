...
// Helper: navega al detalle del primer dron. Devuelve false si no hay drones.
async function gotoFirstDronDetail(page) {
...
test.describe('Stock Dron — Vista de detalle', () => {

  // ── 404 / not found ──────────────────────────────────────────────────────

  test('URL con ID inexistente muestra "Ítem no encontrado"', async ({ page }) => {
    await page.goto('/home/stock/drones/999999999')
    await page.waitForLoadState('networkidle')
    await expect(page.locator('text=Ítem no encontrado')).toBeVisible({ timeout: 8000 })
  })

  test('botón Volver desde 404 lleva a la lista de drones', async ({ page }) => {
    await page.goto('/home/stock/drones/999999999')
    await page.waitForLoadState('networkidle')
    await expect(page.locator('text=Ítem no encontrado')).toBeVisible({ timeout: 8000 })
  ...
  test('detalle de dron carga sin errores', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
  ...
  test('breadcrumb muestra Stock > Drones > nombre del dron', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
    await expect(page.locator('.breadcrumb a', { hasText: 'Stock' })).toBeVisible()
    await expect(page.locator('.breadcrumb a', { hasText: 'Drones' })).toBeVisible()
    await expect(page.locator('.breadcrumb__current')).toBeVisible()
  })

  test('click en "Drones" del breadcrumb navega a la lista', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
    await page.locator('.breadcrumb a', { hasText: 'Drones' }).click()
    await expect(page).toHaveURL(/\/stock\/drones$/, { timeout: 8000 })
  })
  ...
  test('hero muestra badge de estado del dron', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
  ...
  test('hero muestra título del dron', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
    await expect(page.locator('.hero__title')).toBeVisible()
  })

  test('hero muestra subtítulo "Dron"', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
    await expect(page.locator('.hero__subtitle')).toContainText('Dron')
  })

  test('botón "Volver al listado" en hero lleva a /stock/drones', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
  ...
  test('navegación directa por URL al mismo dron carga correctamente', async ({ page }) => {
    await page.goto('/home/stock/drones')
    await page.waitForLoadState('networkidle')
    const card = page.locator('.equip-card').first()
  ...
  test('modal tiene campos de identificación: Nombre, Marca, Modelo, Nº de serie, Garantía', async ({ page }) => {
    ...
    await expect(page.locator('.modal-card input[placeholder="Ej: Matrice 4TD"]')).toBeVisible()
  ...
  test('modal muestra datos pre-cargados del dron al abrirse', async ({ page }) => {
    const ok = await gotoFirstDronDetail(page)
    if (!ok) { test.skip(); return }
    await page.locator('.hero__actions').getByRole('button', { name: /Editar|Completar datos/ }).click()
    await expect(page.locator('.modal-card')).toBeVisible({ timeout: 8000 })
