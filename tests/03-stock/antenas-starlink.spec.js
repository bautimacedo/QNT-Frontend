...
test('lista de antenas Starlink carga sin errores', async ({ page }) => {
  await expect(page.locator('text=Error al cargar')).not.toBeVisible()
  await expect(page.getByRole('heading', { name: /Starlink/i })).toBeVisible()
})
...
test('estado vacío muestra mensaje cuando no hay antenas', async ({ page }) => {
  ...
  if (await card.count() === 0) {
    await expect(page.locator('text=No hay antenas Starlink registradas')).toBeVisible()
    return
  }
...
test('botón Limpiar aparece con filtros activos y los limpia', async ({ page }) => {
  await page.locator('input[placeholder="Marca"]').fill('SpaceX')
...
test('click en card lleva al detalle', async ({ page }) => {
  ...
  await card.click()
  await expect(page).toHaveURL(/\/stock\/antenas-starlink\/\d+/, { timeout: 10000 })
})

test('detalle de antena Starlink muestra estado', async ({ page }) => {
  const card = page.locator('button.equip-card, .equip-card').first()
  if (await card.count() === 0) { test.skip(); return }
  await card.click()
  await page.waitForLoadState('networkidle')
