...
async function gotoFirstAntenaRtkDetail(page) {
...
test.describe('Stock Antena RTK — Vista de detalle', () => {

  // ── 404 / not found ──────────────────────────────────────────────────────

  test('URL con ID inexistente muestra "Ítem no encontrado"', async ({ page }) => {
    await page.goto('/home/stock/antenas-rtk/999999999')
    await page.waitForLoadState('networkidle')
    await expect(page.locator('text=Ítem no encontrado')).toBeVisible({ timeout: 8000 })
  })

  test('botón Volver desde 404 lleva a la lista de antenas RTK', async ({ page }) => {
    await page.goto('/home/stock/antenas-rtk/999999999')
    await page.waitForLoadState('networkidle')
    await expect(page.locator('text=Ítem no encontrado')).toBeVisible({ timeout: 8000 })
    await page.getByRole('button', { name: 'Volver al listado' }).click()
    await expect(page).toHaveURL(/\/stock\/antenas-rtk$/, { timeout: 8000 })
  })

  // ── Carga y estructura general ────────────────────────────────────────────

  test('detalle de antena RTK carga sin errores', async ({ page }) => {
    const ok = await gotoFirstAntenaRtkDetail(page)
  ...
  test('breadcrumb muestra Stock > Antenas RTK > nombre de la antena', async ({ page }) => {
    const ok = await gotoFirstAntenaRtkDetail(page)
    if (!ok) { test.skip(); return }
    await expect(page.locator('.breadcrumb a', { hasText: 'Stock' })).toBeVisible()
    await expect(page.locator('.breadcrumb a', { hasText: 'Antenas RTK' })).toBeVisible()
    await expect(page.locator('.breadcrumb__current')).toBeVisible()
  })

  test('click en "Antenas RTK" del breadcrumb navega a la lista', async ({ page }) => {
    const ok = await gotoFirstAntenaRtkDetail(page)
    if (!ok) { test.skip(); return }
    await page.locator('.breadcrumb a', { hasText: 'Antenas RTK' }).click()
    await expect(page).toHaveURL(/\/stock\/antenas-rtk$/, { timeout: 8000 })
  })

  // ── Hero ─────────────────────────────────────────────────────────────────

  test('hero muestra título de la antena RTK', async ({ page }) => {
    const ok = await gotoFirstAntenaRtkDetail(page)
    if (!ok) { test.skip(); return }
    await expect(page.locator('.hero__title')).toBeVisible()
  })

  test('hero muestra subtítulo "Antena RTK"', async ({ page }) => {
    const ok = await gotoFirstAntenaRtkDetail(page)
    if (!ok) { test.skip(); return }
    await expect(page.locator('.hero__subtitle')).toContainText('Antena RTK')
  })
  ...
  test('botón "Volver al listado" en hero lleva a /stock/antenas-rtk', async ({ page }) => {
    const ok = await gotoFirstAntenaRtkDetail(page)
    if (!ok) { test.skip(); return }
